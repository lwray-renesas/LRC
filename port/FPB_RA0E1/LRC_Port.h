 ...
 */
#define LRC_CRITICAL_SECTION_PREPARE() uint32_t interrupt_save = __get_PRIMASK()

/** @brief Macro used to enter critical section
 * @details Generally disables interrupts.
 */
#define LRC_CRITICAL_SECTION_ENTER() __disable_irq()

/** @brief Macro used to exit critical section
 * @details Generally restores interrupts state.
 */
#define LRC_CRITICAL_SECTION_EXIT() __set_PRIMASK(interrupt_save)
